/**
 * 合规新闻聚合器：只读取 data/sources.json 中启用的公开、可订阅 RSS。
...
type Source = { name: string; type: string; market?: 'global' | 'mainland'; defaultCategory?: string; url: string; feedUrl?: string; tags?: string[]; enabled?: boolean };
...
async function parsePublicFeed(feedUrl: string) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 12_000);
  try {
    const response = await fetch(feedUrl, {
      headers: { 'User-Agent': 'AI-Price-Map/0.1 (public RSS aggregator)' },
...
function slugify(input: string) { return `rss-${createHash('sha256').update(input).digest('hex').slice(0, 24)}`; }
function clip(input = '', length = 72) { return input.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim().slice(0, length); }
function category(title: string, source: Source) {
  ...
  return source.defaultCategory || '产品更新';
}
function chineseSummary(source: Source, item: FeedItem) {
  const subject = clip(item.title || item.contentSnippet || '最新官方更新');
  const verb = source.market === 'mainland' ? '发布公开报道' : '发布官方更新';
  return `${source.name}${verb}，主题为“${subject}”。本页仅整理公开信息，请通过原文链接核验完整内容。`.slice(0, 120);
}
...
for (const source of feeds) {
  try {
    const parsed = await parsePublicFeed(source.feedUrl!);
    for (const item of (parsed.items as FeedItem[]).slice(0, 12)) {
      const publishedAt = item.isoDate || item.pubDate;
      if (!item.link || !publishedAt || seen.has(item.link)) continue;
      seen.add(item.link);
      collected.push({
        slug: slugify(item.link), title: item.title || '官方更新', source: source.name, sourceUrl: item.link,
        publishedAt, companies: source.tags || [source.name],
        category: category(item.title || '', source), market: source.market || 'global', summary: chineseSummary(source, item), importance: 6
      });
    }
...
console.log(`已从 ${feeds.length} 个公开 RSS 获取 ${collected.length} 条新闻`);
